...
def compare_installation():
    ...
    pip_time += measure_command(
        ["python", "-m", "venv", str(pip_venv)], "pip: crear entorno virtual"
    )
    pip_time += measure_command(
        [str(pip_venv / "bin" / "pip"), "install", "requests"], "pip: instalar requests"
    )

    # Test con uv
    uv_time = 0
    uv_time += measure_command(["uv", "venv", str(uv_venv)], "uv: crear entorno virtual")
